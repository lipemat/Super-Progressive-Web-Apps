 ...
 *
 * @param string $slug is the $key used in the $addons array in superpwa_get_addons().
 * 		For add-ons installed as a separate plugin, this will be plugin-directory/main-plugin-file.php
 *
 * @return (string) one of the statuses as described above. '' if $slug is not a valid add-on.
 *
 * @since 1.7
 */
function superpwa_addons_status( $slug ) : string {
	...
	switch( $addon['type'] ) {
		...
		case 'bundled':
			...
			if ( is_array( $active_addons ) && in_array( $slug, $active_addons, true ) ) {
				return 'active';
			}

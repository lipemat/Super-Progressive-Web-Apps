...
function superpwa_multisite_activation_status( $status ) {
	...
	// Get current list of sites where PWA is activated.
 ...
 * This also this gives the user an option to decide if PWA should handle this by changing the defenition of wp_is_large_network.
...
function superpwa_multisite_network_deactivator() {
	...
	// Retrieve the list of blog ids where PWA is active. (saved with blog_id as $key and activation_status as $value)
	...
	foreach( $superpwa_sites as $blog_id => $actviation_status ) {
		...
		/**
		 * Delete PWA version info for current blog.
		 *
		 * This is required so that superpwa_upgrader() will run and create the manifest and service worker on next activation.
		 * Known edge case: Database upgrade that relies on the version number will fail if user deactivates and later activates after PWA is updated.
		 */
